...
export class ExpertRoutingResolveService implements Resolve<IExpert> {
  constructor(protected service: ExpertService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IExpert> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((expert: HttpResponse<Expert>) => {
          if (expert.body) {
